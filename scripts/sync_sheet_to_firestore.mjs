...
const SHEET_NAME = process.env.RESKILLING_VIDEO_SHEET_NAME || '\u7206\u901f\u30ea\u30b9\u30ad\u30ea\u30f3\u30b0\u52d5\u753b';
...
function rowToContent(row, index) { const [uploadedAt, title, url, note] = row.map((cell) => String(cell || '').trim()); if (!title || !url) return null; return { title, url, note: note || '', uploadedAt: normalizeDate(uploadedAt), publishedAt: normalizeDate(uploadedAt), category: categoryFor(title), type: 'video', published: true, order: index + 1, updatedAt: admin.firestore.FieldValue.serverTimestamp() }; }
